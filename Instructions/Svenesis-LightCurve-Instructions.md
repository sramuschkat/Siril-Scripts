...
**Version 1.0.14** | Siril Python Script for Exoplanet Transit Photometry
...
| **Star coordinates measured, not assumed** | Whether Siril's star y is a file row or a flipped one depends on whether the plate solve flipped the reference frame. Since v1.0.12 the script decides this on the reference frame's own pixels after detection (which reading puts the detected stars on stars) and uses the verdict for every aperture seed, the saturation box and the field image. On a TheSkyX set whose solve failed, the old fixed assumption had put every aperture on empty sky |
| **Site and name from the header, whatever the spelling** | SITELAT/SITELONG written as `+38 55 48.51` are read as degrees, so the altitude-based longitude sign check can flip a west-positive value instead of the form's site silently taking over. A label in front of the designation (`ESPC WASP-12`) is dropped one word at a time until the archive answers, and a retry that fails says so. Since v1.0.13 the **header's site wins** when the frames carry one; the form is the fallback for frames without, or the override when *Use the form's site even when the headers carry one* is ticked in group 5. A form site more than 50 km from the header's is reported either way: airmass and BJD both follow the site in use. A site read from the frames is written into the two fields, overwriting what was there |
...
**The log says what it is about to do.** Each master build (`building bias master from 100 frame(s): link, stack …`) and the calibrate of the lights (`Calibrating 223 light(s): calibrate lights "-flat=…"`) get a line *before* the work starts, so the long stretch of Siril's own `Reading FITS` / `Saving FITS` lines has a caption; the summaries (`built bias master …`, `Calibrated with …`) still follow once it is done.
...
3. **Site coordinates** — only needed when the frames carry no site (SITELAT/SITELONG); a header site wins unless the checkbox in group 5 forces the form. Without either, the airmass detrend is skipped and the report says so.
...
- **The transit is no longer eaten *before* the fit sees it.** A sequential detrend subtracts whatever a basis can explain, transit included; in one design the two are solved together and the depth survives at 11.2 mmag of 12.0 against a basis shaped like the transit. Until v1.0.12 this page went further and claimed the transit *cannot* be absorbed into a correlated basis. That was wrong, and a real night showed how: a basis that looks like the transit is degenerate with it in the same matrix too, and the solve splits the depth between them with an error bar to match. What stands between a basis and the transit since v1.0.13 is described below: the anchor, the BIC and the comparison stars.
...
**Anchored, selected and cross-checked, since v1.0.13.** Three rules, the ones TFOP's follow-up guidelines and AIJ's users apply, decide what the fit may do with a known planet:

1. **Anchored on the ephemeris.** When the archive knows the planet, the mid-time is searched only within a window round the prediction — the ephemeris error grown over the epochs, times three, never under 30 minutes, because the archive's timing error is often missing — and the duration within 15 % of the archive's first-to-fourth contact (or the orbit's, from a/R★ and inclination); the period is fixed. Only the depth and the systematics are free, which is how EXOTIC and TFOP fit a known planet. A free mid-time and duration remain the right tool for a signal nobody has an ephemeris for, and the free scan still runs on every anchored fit as a cross-check, logged and reported. On JD_UTC times there is no anchor: an archive T0 is BJD_TDB and would be minutes off.
2. **Bases by the BIC.** Airmass, and the meridian-flip step when a flip was seen, are always in. Seeing, sky, star count and the target's x/y drift each enter only when they lower the Bayesian Information Criterion by 2 or more (Kass & Raftery's "positive evidence") with the transit in the model — forward selection, best candidate first, one log line with every delta. Why this matters was measured on a 2026-08-31 night of TOI-2040.01: with all six bases offered unasked and a free duration, a 7 px drift of the star in x stretched the 2.6 h transit to 3.5 h and took a 6 σ detection to 1 σ without lowering the residual scatter at all. Anchored, the BIC rejects the same basis (+4.9) and the fit gives (Rp/R★)² = 1.42 % against TESS's 1.47 %, the mid-time 0.1 min from the prediction. On EXOTIC's HAT-P-32 set the x drift earns its place (−3.0) and stays.
3. **A kept basis is measured on the comparison stars.** An instrumental dependence — a flat-field residual, a pixel-phase effect on an undersampled star — moves every star that drifts across the same pixels; a transit moves one. Each active comparison is fitted against the ensemble of the others, on its own centroid for the x/y bases and on the frame's value for the rest, with the airmass column alongside, and the target's slope must sit within two sigma of what the ensemble shows. A dependence only the target has is called out in red: a basis only the target follows is fitting the transit.
...
Six radii from **0.75 to 2.5 × FWHM** are each photometered through Siril's own `setphot` + `light_curve`, and the one with the least robust scatter wins. The number of measured frames is one tie-breaker: an aperture that scores well by measuring fewer frames has not won anything. The other, since v1.0.14, is size: within 3 % of the least scatter the larger radius is taken, for the reason §4 gives.
...
**And chosen by the fit, since v1.0.13.** EXOTIC keeps the one comparison star whose fitted light curve leaves the smallest residual; a HOPS or AIJ user looks at each comparison's curve and unticks the bad one. This is the automated ensemble version of both: after the photometry, worst first, a comparison star is dropped while the anchored transit fit's residual scatter falls by 2 % or more without it, and at least two always stay. The log names each drop with the residual before and after, and the field image and the exports show the ensemble that was used. Why the scatter test above is not enough was measured on EXOTIC's HAT-P-32 set: a bright comparison star with 27 mmag of *slow* structure passed the 3× veto, carried a third of the ensemble, and took Rp/R★ from 0.156 to 0.164 against EXOTIC's 0.154. Its own scatter was unremarkable; only the fit saw what it did to the depth.
...
**A TESS candidate will be refused by AAVSO's form.** The form validates `#EXOPLANET_NAME` against the archive's *confirmed*-planet table; a TOI such as TOI-7425.01 lives in the TOI table only, so the form answers "not found — missing a suffix letter?". There is no letter to add: the candidate is not a named planet yet. The log and the file's `#NOTES` say so and point at ExoFOP-TESS, where candidate follow-up is submitted.
...
| `EXOTIC/` | The same run as EXOTIC's output folder: `AAVSO_<planet>_<date>.txt`, `FinalLightCurve_<planet>_<date>.png/.pdf`, and `temp/` with the rest (see below) |
| `HOPS/` | The same run as HOPS's observation folder: `PHOTOMETRY_1/`, its `PHOTOMETRY_APERTURE_FITTING/` in HOPS mode, and the ExoClock/ETD export files (see below) |

**The EXOTIC and HOPS folders** exist so that anything built to read those pipelines' output reads this script's — same folder layout, same file names, same column orders; only the numbers are this script's. `EXOTIC/` holds `AAVSO_<planet>_<date>.txt` with EXOTIC's full header set (`#COMP_STAR-XC`, `#FILTER-XC`, `#PRIORS-XC`, `#RESULTS-XC` as JSON, the date spelled `15-October-2025` as EXOTIC's template does), `FinalLightCurve_….png/.pdf`, and in `temp/`: `FinalLightCurve_….csv` (BJD_TDB, phase, flux, uncertainty, model, airmass), `FinalParams_….json`, `NormalizedFlux_….txt` and its plot, `TargetRawFlux`/`CompRawFlux` plots, `CentroidPositions&Distances_….pdf`, `Observing_Statistics_<star>_….png/.pdf` per star, `FOV_…_AsinhStretch.png/.pdf`, `Triangle_….png` (the posterior's corner plot, HOPS mode only) and `PlateStatus_….csv` (one row per frame, one column per reason a frame gave no measurement). The parameters come from the HOPS-mode posterior when the run has one — it is the ephemeris-locked fit EXOTIC's own is — else from the blind fit; the JSON's `Fit` entry says which. A TESS candidate has no a/R★ or inclination in the archive; `#PRIORS` and `#RESULTS` then quote the orbit the HOPS-mode fit derived from the duration, not blanks. Am1/Am2 are labelled with their law in `#NOTES` and in the JSON's `Airmass model`: EXOTIC's own are `a1·exp(a2·X)`, the HOPS-mode fit is linear in the airmass above its minimum, the blind fit's magnitude slope is turned into the exponent. `#OBSTYPE` comes from the **Camera** control beside the observer code (CCD, also for a CMOS camera, or DSLR). `HOPS/PHOTOMETRY_1/` holds `PHOTOMETRY_APERTURE.txt` (exposure-**start** JD_UTC, target over the raw sum of the active comparisons, error — the file ExoClock takes), `PHOTOMETRY_a.txt` (file name, time, then every star's x, y, flux, error, background inside the aperture and its error, in counts as HOPS writes them), `FOV.pdf`, `RESULTS.pdf` (T and each C_i relative curve), `ExoClock_info.txt`, `photometry_output_description.txt` and `log.yaml`; in HOPS-compatible mode `PHOTOMETRY_APERTURE_FITTING/` adds `results.txt`, `model.txt`, `detrended_model.txt`, `corner.pdf`, `traces.pdf`, `detrended_model.jpg` and their description; beside `PHOTOMETRY_1` sit `HOPS_PHOTOMETRY_1_PHOTOMETRY_APERTURE_<date>_<planet>_<filter>_<exposure>s_for_ExoClock.txt` and `…_for_ETD.txt` (ETD's times shifted to mid-exposure as HOPS's export does; the errors stay physical — HOPS divides them by √gain "to be in electrons", but the curve is a ratio and the gain is already inside the CCD equation, so that only shrank the bars). Not written, and said so in the log: HOPS's PSF-photometry files (`PHOTOMETRY_GAUSS.txt`, `PHOTOMETRY_g.txt` — this script has no PSF fitting), EXOTIC's `AID_AAVSO_….txt` (it needs AAVSO chart comparison stars with AUIDs), and the corner plot on a run without a HOPS-mode posterior. The per-star files need the script's own photometry engine; when Siril's `light_curve` measured the run, the HOPS curve is made from the differential magnitudes and the per-star files are skipped.
