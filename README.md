...
| [Svenesis LightCurve](#svenesis-lightcurve) | Exoplanet transit photometry inside Siril, in the spirit of [EXOTIC](https://github.com/rzellem/EXOTIC) and [HOPS](https://github.com/ExoWorldsSpies/hops): point it at a folder of sub-exposures and it measures the flux itself — follow-star re-centroiding, subpixel apertures, comparison stars kept by measured scatter — while Siril calibrates, registers (no resampling) and plate-solves. Limb-darkened transit fit with a calibrated significance floor, O−C against the published ephemeris, CSV/PNG/report and an AAVSO Exoplanet Watch file. **Tested on EXOTIC's own sample data with matching results:** Rp/R★ 0.1554 ± 0.0059 vs EXOTIC's published 0.1541 ± 0.0033 (0.2 σ). | [Guide](Instructions/Svenesis-LightCurve-Instructions.md) · [DE](Instructions/Svenesis-LightCurve-Instructions_de.md) | — |
...
**File:** `Svenesis-LightCurve.py` (v1.0.11) — **[Detailed Instructions](Instructions/Svenesis-LightCurve-Instructions.md)** · **[Deutsche Anleitung](Instructions/Svenesis-LightCurve-Instructions_de.md)**
...
**The script measures the flux itself**, the way EXOTIC and HOPS do: every star re-centroided per frame from its registration-predicted position (the "follow star" Siril's `light_curve` lacks), subpixel circular apertures against a sigma-clipped sky annulus, the aperture chosen by point-to-point noise, comparison stars kept or dropped by their *measured* scatter, errors from the CCD equation with every term measured. Measured on the same drifting 142-frame run: this engine keeps 140 points at 6.3 mmag where Siril's `light_curve` kept 67 — and `light_curve` remains intact as the loud fallback. The script also does the parts nobody's pixels decide: which star is the target, how to remove the airmass ramp without eating the transit depth — and whether to claim anything at all. Frames are sequenced by `DATE-OBS`, not by file name (N.I.N.A. puts the sensor temperature in the name, and `-10.10C` sorts after `-10.00C` — five mid-run frames once landed at the end and read as a second flip), and the time-stamp convention is checked against `DATE-AVG` or `DATE-END` rather than assumed: a program that stamps mid-exposure gets no half-exposure added.

**Validated against EXOTIC on its own sample data** (HAT-P-32 b, 142 frames): Rp/R★ = 0.1554 ± 0.0059 against EXOTIC's published 0.1541 ± 0.0033 — 0.2 σ apart (as of v1.0.6). Both depth conventions are reported and labelled: the limb-darkened central depth the fit measures, and the (Rp/R★)² that EXOTIC, HOPS and AstroImageJ quote (~20 % shallower on a solar-type star — compare *that* one with theirs and with the archive).

**Why not just Siril's own [light-curve tool](https://siril.readthedocs.io/en/stable/photometry/lightcurves.html)?** That workflow produces a light curve; this script produces a measurement — follow-star re-centroiding, drift handling and comp screening that `light_curve` lacks (140 of 142 points where it kept 67 on the same run), plus the analysis the native tool ends before: BJD_TDB, a simultaneous limb-darkened fit, calibrated significance, O−C and the AAVSO file. The full comparison is in the [manual's FAQ](Instructions/Svenesis-LightCurve-Instructions.md#14-faq).

**Calibration finds its own frames.** Point at the subs — or at any folder above them, the scan is recursive and sorts lights from calibration frames by header — and, once, at the folder where your reusable darks live; the flats are found beside the lights in the N.I.N.A. `LIGHT`/`FLAT` layout, grouped by exposure/gain/temperature/binning/size/camera, stacked into masters and cached for the next run. The pixel work is Siril's `calibrate` — there is no bias/dark/flat arithmetic in the script, for the same reason there is no photometry in it. What gets refused is said out loud, because a master that was found and rejected leaves a run that looks exactly like one where no master existed: a dark at the wrong exposure is named with what the mismatch would have done, darks are split by temperature and bias is not, and bias is never applied together with a dark — the dark already carries the offset, so it corrects the flats instead. Temperature is demanded only where the thermal signal is what the master removes (dark against light, flat-dark against flat): a flat is a ratio and a bias is read noise, so a flat shot on a warmer night still calibrates.

**Taken from EXOTIC, measured before and after.** The aperture is now *chosen* — six radii from 0.75 to 2.5 FWHM, each photometered, least scatter wins — instead of whatever `-autoring` derived. Comparison stars are photometered against each other and dropped on their own variability. And there is spike rejection, which mattered more than it looks: one 100 mmag outlier on a real 12 mmag transit took the significance from **12.1σ to 3.2σ**, under the detection floor, so a measured transit read as *not claimed*. A robust post-fit scatter recovers 6.9σ of that and removing the point recovers the rest. The run also writes an AAVSO Exoplanet Watch file — refused unless the times are BJD_TDB.
...
**Clipping is not variability.** A comparison star whose core intermittently hits the sensor's (or, on calibrated floats, Siril's [0, 1]) ceiling scatters exactly like a variable star, and on the run that exposed this, 73 of 223 points vanished from the curve without a word. Three guards stand there now: a comp whose reference peak already sits at 70 % of the clip level is dropped up front and the next ranked star with headroom is promoted from the selection's reserve; every comp that still clips is listed with its clip count; and the "N points measured" line names every missing frame with its reason. On the same dataset the guards turned a silent 150-point non-detection into a 223-point detection whose depth matches TESS — and revealed that saturated comps, not the missing flats, had been bending both the depth and the transit time.

**Two questions, two modes.** The blind fit asks *is there a transit?* — mid-time, duration and shape free, a calibrated significance deciding. HOPS (ExoWorldsSpies, the pipeline behind ExoClock) asks *given the catalogue's planet, how deep and when?* Since v1.0.7 a dropdown in the Analysis group offers both. **HOPS-compatible mode** takes the orbit from the archive (a/R★, inclination, eccentricity, periastron), so duration and ingress are physics rather than parameters; fits only Rp/R★, the mid-time within ±0.2 d of the predicted epoch, a normalisation and HOPS's detrending (airmass, time or time², multiplied into the flux, plus the meridian-flip step when one was detected); measures what HOPS measures (the target over the raw sum of the comparison stars, errors propagated as HOPS does), averages the model over each exposure in 10 s sub-steps as HOPS does, uses four Claret limb-darkening coefficients (computed here with ExoTETHyS's method on Phoenix 2018 model atmospheres, reproducing ExoTETHyS's own output to 1e-7, or yours, or the quadratic defaults rewritten exactly; an unfiltered run uses HOPS's own clear/luminance passband from pylightcurve's photometry database, MIT licence, fetched at run time); removes outliers and rescales the error bars the way HOPS does; and samples the posterior with the same affine-invariant ensemble algorithm emcee implements, reporting the 16/50/84 percentiles. The occultation integral is analytic — pylightcurve's formulation, sector plus lunes with closed-form radial integrals and one 30-node quadrature for the arc term — reproducing pylightcurve's own function to 1e-15 and the ring integration kept as reference to 3e-6, at a quarter of the cost; the orbit matches pylightcurve's to 1e-14; the transit duration finds the contacts on the actual orbit, and the whole mode head to head against pylightcurve's own fitting class with emcee on the same data: outlier count and scale factor identical, every parameter within 0.1 σ. The sampler is seeded, so a rerun repeats. What does not change: the blind test still runs first and alone decides whether a transit is *claimed* — HOPS mode measures the catalogue's planet, it does not test for one. When a fitted contact lies outside the run, the log, `results.txt` and the report say so: there a transit and a baseline offset are the same curve, and the numbers do not measure the planet.

### The chart tells the whole story

The legend quotes T0 and Rp/R★ with errors and names the detrending bases; spike-rejected points appear as red crosses instead of vanishing; per-point error bars have an on/off switch; the residual panel reports its STD and lag-1 autocorrelation with a verdict. Beside the fitted model, the **expected transit from the archive ephemeris** is drawn *whether or not the fit claimed anything* — on a detection the shift between the curves **is** the O−C, quoted in minutes with its error; on a non-detection the legend says whether a transit was even due in your window, or names the nearest mid-transit in hours from the run. Its depth is the archive's Rp/R★, or for a TESS candidate the SPOC depth inverted through the same limb-darkened model that draws the curve, not its square root. **TESS candidates** (`TOI-XXXX.NN`) get their ephemeris from the archive's TOI list when the confirmed-planet table cannot know them, with the TFOPWG disposition said out loud and a red warning on false positives. And the chart speaks your planning tool's language: a wall-clock axis across the top (local time from N.I.N.A.'s `DATE-LOC`, UTC otherwise — the axis says which), the predicted start/mid/end contacts stamped in clock time, and the meridian flip drawn as a dashed marker at the moment the field turned. One grammar throughout: orange dashed is the flip, cyan dotted are contacts, and coloured dashed lines exist only for a claimed detection. On a detection the comparison is spelled out contact by contact — the measured start/end with their own clock stamps, a duration arrow on the expected dip with Δduration in its label, and grey Δ spans quoting each contact's offset in minutes (measured − predicted, the O−C sign convention).

### Output

`lightcurve/lightcurve.csv` (JD, raw, centred, detrended, error, airmass), the plot as PNG — and, when the times are BJD_TDB, an `AAVSO_exoplanet.txt` in Exoplanet Watch's format and EXOTIC's layout — the four fields the upload form requires (`STAR_NAME`, `EXOPLANET_NAME`, `EXPOSURE_TIME`, `MEASUREMENT_TYPE=Rnflux`), DIFF as relative normalised flux with the airmass and the fitted systematics model as detrend columns, `#PRIORS`/`#RESULTS`, AAVSO's filter code from the form or the frames (a RED wheel is `TR`), plus T0 ± error, both depth conventions, Rp/R★ ± error and duration in the header. `field.png` is the picture the form asks for beside it: the plate-solved reference frame with the target and the comparison stars marked, north/east and a 5′ bar, under 2 MB. A TESS candidate is flagged in the log and in `#NOTES`: AAVSO's form validates the planet name against the confirmed-planet table and refuses a TOI until it is confirmed; ExoFOP-TESS is the venue for candidates. **Two more folders hold the same run in the other pipelines' own layouts:** `lightcurve/EXOTIC/` is what EXOTIC leaves in its output folder — `AAVSO_<planet>_<date>.txt` with EXOTIC's header set including the `-XC` JSON lines, `FinalLightCurve_…png/pdf`, and a `temp/` with the final time series as CSV, `FinalParams_….json`, the FOV plot, the normalised-flux table and plot, raw-flux, centroid and observing-statistics plots, the posterior's `Triangle_….png` and a `PlateStatus_….csv` — and `lightcurve/HOPS/` is HOPS's observation folder: `PHOTOMETRY_1/` with `PHOTOMETRY_APERTURE.txt` (exposure-start JD_UTC, target over the comparison sum, error), `PHOTOMETRY_a.txt` (every star's position, flux, error and sky per frame), `FOV.pdf`, `RESULTS.pdf`, `ExoClock_info.txt` and `log.yaml`; in HOPS-compatible mode `PHOTOMETRY_APERTURE_FITTING/` with `results.txt`, `model.txt`, `detrended_model.txt`, `corner.pdf`, `traces.pdf` and `detrended_model.jpg`; and the ExoClock and ETD export files under HOPS's own name pattern. What a folder cannot hold (PSF-photometry files, the AID file that needs AAVSO chart stars, a corner plot without a HOPS-mode posterior) is named in the log rather than faked; for a TESS candidate the EXOTIC file quotes a/R★ and inclination from the orbit the HOPS-mode fit derived, not blanks. Each master build and the calibrate of the lights announce themselves in the log before the work starts. The **Save results** button writes two files in one click: `results.txt` in the exact layout HOPS leaves in its fitting folder (the column-aligned parameter table with covariance error bars, the `#Filter`/`#Epoch` block, both residual-statistics blocks including a pure-numpy Shapiro–Wilk W verified against scipy — anything that parses a HOPS `results.txt` parses this one unchanged; in HOPS-compatible mode it is HOPS's own table — a₁..a₄, sma_over_rs, inclination, the real outlier count and scale factor, asymmetric posterior bars), and `report.txt`, the full plain-text report with the comparison stars, every rejection and its reason, the method, and the result.

### Tests

`tests/test_lightcurve_helpers.py` runs with plain `python3` — no Siril required. Nearly 800 checks against input with a known answer: the J2000 epoch, sec z, synthetic transits of stated depths recovered by the full photometry engine and the full fit, twelve pure-noise runs that must not be claimed, the monotonic ramp that the two-sided test exists for, error bars calibrated against 24 independent synthetic nights, and the limb-darkened depth conventions round-tripped through the same model the fit uses.
