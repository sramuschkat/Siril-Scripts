...
| [Svenesis LightCurve](#svenesis-lightcurve) | Measure an exoplanet transit light curve from a folder of sub-exposures: star-following aperture photometry measured by the script itself over a registered-but-not-resampled sequence (every star re-centroided per frame, subpixel apertures, comps kept by measured scatter — Siril's `light_curve` remains as the fallback), calibration frames discovered beside the lights and stacked into masters through Siril's own `calibrate`, comparison stars filtered on SNR / saturation / separation / isolation, the target read straight out of your headers (OBJCTRA/OBJCTDEC from the lights, 5.7″ from the archive on our test run) with the published ephemeris fetched by name, a limb-darkened transit fitted SIMULTANEOUSLY with airmass, seeing, sky and star count, O−C against the published ephemeris, and a two-sided significance test that refuses to report a monotonic trend as a transit. CSV, PNG and plain-text report. | [Guide](Instructions/Svenesis-LightCurve-Instructions.md) · [DE](Instructions/Svenesis-LightCurve-Instructions_de.md) | — |
